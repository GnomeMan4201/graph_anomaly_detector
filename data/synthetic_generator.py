...
# Realistic hour-of-day activity weights (0-23h UTC)
_HOURLY_WEIGHT = [
    0.2, 0.1, 0.1, 0.1, 0.1, 0.2,   # 00-05 (low)
    0.4, 0.7, 0.9, 1.0, 1.0, 0.9,   # 06-11 (morning rise)
    0.8, 0.9, 0.9, 0.8, 0.7, 0.9,   # 12-17 (midday plateau)
...
class SyntheticDataGenerator:
    ...
    def __init__(
        ...
        # Assign user id namespaces
        self._uid = 0

        self.normal_users : List[str] = self._make_ids("U", n_normal_users)
        self.target_users : List[str] = self._make_ids("T", n_target_users)

        # Bot clusters: list of (cluster_id, [bot_ids], [victim_ids])
        all_users = self.normal_users + self.target_users
    ...
    def _normal_events(self) -> List[Dict]:
        ...
        for user in self.normal_users:
            ...
            for day in range(self.n_days):
                ...
                # Burst injection for burst users
    ...
    def _bot_cluster_events(self) -> List[Dict]:
        """
        Bot coordinated behaviour:
         - All bots in a cluster fire at the same time ± burst_jitter_s.
         - They target the same victim set repeatedly.
         - Action type is homogeneous within a burst (follow or like).
         - Multiple burst rounds per day.
        """
        events = []
        for cluster_id, bots, victims in self.bot_clusters:
            for day in range(self.n_days):
                # 2-5 synchronised burst rounds per day
                    ...
                    for bot in bots:
                        for victim in victims:
                            ...
                            events.append({
                                ...
                                "_cluster":    cluster_id,   # ground-truth label
                            })
        return events
    ...
    def save_jsonl(self, path: str) -> int:
        """Write events to a .jsonl file. Returns record count."""
        events = self.generate()
        p = Path(path)
        p.parent.mkdir(parents=True, exist_ok=True)
        with p.open("w") as f:
            for e in events:
                # Strip internal labels before writing

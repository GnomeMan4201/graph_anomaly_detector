...
Derived from actual signals in the DEV.to / GitHub investigations:
  ...
  follow_ratio            : following / (followers + 1) — bots push this high
...
class SocialFeatureExtractor:
    ...
    def extract(self, nodes_df: pd.DataFrame) -> pd.DataFrame:
        ...
        feats["username_hash_suffix"] = self._col(
            nodes_df, "username_hash_suffix"
        ).astype(float)
    ...
    def _profile_completeness(self, df: pd.DataFrame) -> pd.Series:
        """Score 0–1 for populated bio, avatar, article, and comment fields."""
    ...
    def _s3_batch_density(self, s3_series: pd.Series) -> pd.Series:
        """Count nearby S3 IDs within ±batch_window and normalise by population."""
        ...
        for i in valid_idx:
            lo = np.searchsorted(
                valid_ids, s3[i] - self.batch_window, side="left"
            )
            hi = np.searchsorted(
                valid_ids, s3[i] + self.batch_window, side="right"
            )
            density[i] = (hi - lo - 1) / max(n, 1)

        return pd.Series(density, index=s3_series.index)

    def _account_age(self, ts_series: pd.Series) -> pd.Series:
        """Return a freshness score where recent accounts are closer to 1.0."""
        import time
    ...
    def _creation_wave(self, ts_series: pd.Series) -> pd.Series:
        """Cluster join timestamps with KMeans and normalise cluster IDs."""
